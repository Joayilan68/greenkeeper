...
export default function Admin() {
  ...
  // Accès réservé aux admins (email Clerk vérifié) — plus de code secret
  useEffect(() => {
  ...
  // ── GARDE D'ACCÈS — admin via email Clerk uniquement ───────────────────────
  // Pendant la vérification : écran de chargement. Si non-admin : redirection (gérée
  // par le useEffect ci-dessus), on n'affiche rien le temps de rediriger.
  if (isLoading) return (
    <div style={{ ...appShell, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"32px 24px" }}>
      <div style={{ fontSize:52 }}>🔐</div>
      <div style={{ fontSize:13, color:"#81c784", marginTop:12 }}>Vérification de l'accès…</div>
    </div>
  );
