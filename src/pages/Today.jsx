...
const ACTION_TO_GP = {
  ...
  "Regarnissage 🌾":        "semences",import { useState, useEffect, useCallback } from "react";
import { useNavigate } from "react-router-dom";
import { useWeather } from "../lib/useWeather";
import { useProfile } from "../lib/useProfile";
import { useHistory } from "../lib/useHistory";
...
export default function Today() {
  ...
  // ── Log action — toujours accessible Free + Premium ──────────────────────
  // Les Free users DOIVENT pouvoir journaliser pour gagner des GreenPoints
  const log = (action) => {
    addEntry(action);
      ...
      <div style={scroll}>
        ...
        <div style={card()}>
          ...
          {!isPaid ? (
            ...
            <div style={{fontSize:13,color:"#81c784",textAlign:"center",padding:"12px 0"}}>{!weather ? "Activez la géolocalisation" : "Appuyez sur ↻"}</div>
          )}
        </div>
        ...
        {/* ── Journaliser — accessible Free ET Premium ───────────────────── */}
        <div style={card()}>
          <div style={cardTitle}>
          ...
          {(arros || plan?.engrais) && (
            <div style={{ marginBottom:12, padding:"10px 12px", background:"rgba(76,175,80,0.08)", borderRadius:12, border:"1px solid rgba(76,175,80,0.2)" }}>
              ...
              ].slice(0,3).map((action, i, arr) => (
                <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 0", borderBottom: i < arr.length-1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
          ...
          {/* Toutes les actions — Free ET Premium */}
          ...
          {/* Invitation Premium pour historique illimité */}
          {isFree && (
            <div style={{ marginTop:12, padding:"8px 12px", background:"rgba(249,168,37,0.08)", border:"1px solid rgba(249,168,37,0.2)", borderRadius:10, textAlign:"center" }}>
              <div style={{ fontSize:11, color:"#f9a825" }}>
                🔒 Historique limité · <span style={{ cursor:"pointer", textDecoration:"underline" }} onClick={() => navigate("/subscribe")}>Premium = historique illimité + IA</span>
              </div>
            </div>
          )}
        </div>
