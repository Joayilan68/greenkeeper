...
import { useRecommandations, getConseilApresAction } from "../lib/useRecommandations";
...
// Mapping action label → clé GreenPoints
const ACTION_TO_GP = {
  "Tonte ✂️":               "tonte",
  "Arrosage 💧":            "arrosage",
  "Engrais 🌱":             "engrais",
  ...
  "Top-dressing 🏖️":       "aeration",   // action premium, même logique
  "Traitement fongicide 💊":"anti_mousse",
  "Scarification 🔩":       "scarification",
};

// Mapping action label → clé conseil agronomique
...
export default function Today() {
  ...
  // ── Nouveaux hooks GreenPoints + Streak + Recommandations ───────────────────
  ...
  const { recommandationPrincipale } = useRecommandations(profile, null, weather);
  const [toastPoints, setToastPoints] = useState(null);

  // Enregistrer la connexion quotidienne au chargement
  useEffect(() => {
  ...
  // ── Log action — version enrichie avec GreenPoints ──────────────────────────
  const log = (action) => {
    if (!canLog) return;
    addEntry(action);
  ...
  return (
    <div>
      ...
      <div style={scroll}>
        ...
        {streak > 0 && (
          <div style={{
            background:   enDanger ? "#fff3e0" : "#e8f5e9",
            border:       `1px solid ${enDanger ? "#f9a825" : "#a5d6a7"}`,
            borderRadius: 12,
            padding:      "10px 14px",
            display:      "flex",
            alignItems:   "center",
            ...
            <div style={{flex:1}}>
              <div style={{fontWeight:600,color:enDanger?"#e65100":"#2e7d32",fontSize:14}}>
                {streakMsg}
              </div>
              {enDanger && (
                <div style={{fontSize:11,color:"#f57c00"}}>
                  Connecte-toi aujourd'hui pour garder ton streak !
                </div>
              )}
            </div>
            <div style={{
        ...
        {/* ── Journaliser — enrichi avec GreenPoints ────────────────────────── */}
        <div style={card()}>
          <div style={cardTitle}>
          ...
          {!canLog ? (
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {ACTIONS.map(a => (
                ...
                >
                  {logged.includes(a) ? `✓ +${ACTION_TO_GP[a] ? "pts" : ""}` : a}
                </button>
              ))}
            </div>
          )}
        ...
        {/* ── Recommandation produit (nouveau) ─────────────────────────────── */}
        {recommandationPrincipale && (
          <div style={{
            ...
            <div style={cardTitle}>
              ...
              <span style={{
                fontSize:  11,
                color:     recommandationPrincipale.urgence === "haute" ? "#c62828" : "#f57f17",
                fontWeight: 600,
              }}>
                {recommandationPrincipale.urgence === "haute" ? "🔴 Maintenant" : "🟡 Bientôt"}
              </span>
            </div>
            <div style={{fontWeight:700,color:"#e8f5e9",fontSize:14,marginBottom:6}}>
              {recommandationPrincipale.label}
            </div>
            <div style={{fontSize:12,color:"#81c784",lineHeight:1.5,marginBottom:12}}>
              {recommandationPrincipale.message(0)}
            </div>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <span style={{
                ...
                📈 {recommandationPrincipale.impact_score}
              </span>
              <button
      ...
      {/* ── Toast GreenPoints (fixe, au-dessus de la nav) ─────────────────── */}
      {toastPoints && (
        <div style={{
          position:     "fixed",
          bottom:       80,
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontWeight:700,color:"#a5d6a7",fontSize:16}}>
                +{toastPoints.points} GreenPoints !
              </div>
              <div style={{color:"#81c784",fontSize:12}}>
                Total : {toastPoints.total?.toLocaleString("fr-FR")} pts · {toastPoints.palier?.icone} {toastPoints.palier?.label}
              </div>
            </div>
          </div>
          {toastPoints.conseil && (
