...
function OptionCard({ selected, onClick, icon, label, desc }) {
  return (
    <div onClick={onClick} style={{
      ...
      {selected && <span style={{ marginLeft:"auto", color:"#43a047", fontSize:18 }}>✓</span>}
    </div>
  );
}
...
export default function OnboardingModal({ onComplete }) {
  ...
  const handleFinish = () => {
    ...
    try { localStorage.setItem("gk_profile", JSON.stringify(profile)); } catch {}
    try { localStorage.setItem("gk_onboarding_done", "true"); } catch {}
    onComplete(profile);
  };
      ...
      <div style={{
        ...
        {step === 1 && (
          <div>
            <div style={{ textAlign:"center", marginBottom:24 }}>
              ...
              <div style={{ fontSize:22, fontWeight:800, color:"#a5d6a7", marginBottom:6 }}>
                Bienvenue sur MG360 !
              </div>
        ...
        {step === 3 && (
          <div>
            ...
            <div style={{ fontSize:12, color:"#81c784", marginBottom:20 }}>
              Utilisée pour la météo locale et les conseils saisonniers adaptés à votre région.
            </div>

            <button onClick={detectLocation} disabled={locLoading} style={{ ...btn.primary, marginBottom:8, fontSize:14, padding:"14px" }}>
              {locLoading ? "⌛ Détection..." : "📍 Autoriser ma localisation"}
            </button>
            <div style={{ fontSize:11, color:"#4a7c5c", textAlign:"center", marginBottom:24 }}>
              Utilisée uniquement pour la météo — jamais partagée
            </div>

            {/* Résumé */}
            <div style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(165,214,167,0.2)", borderRadius:16, padding:16, marginBottom:24 }}>
              <div style={{ fontSize:13, fontWeight:700, color:"#a5d6a7", marginBottom:12 }}>✅ Votre profil MG360</div>
              {[
            ...
            <div style={{ background:"rgba(67,160,71,0.1)", border:"1px solid rgba(67,160,71,0.25)", borderRadius:12, padding:"12px 14px", marginBottom:20, fontSize:12, color:"#a5d6a7", lineHeight:1.6 }}>
              🌿 Votre score de santé est calculé en temps réel grâce à ces informations, la météo locale et votre historique d'entretien.
            </div>
